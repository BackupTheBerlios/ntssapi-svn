...
if ( $searchtype == 'simple' ) {
	...
	if ( $searchby == 'username' ) {
			...
			" as user WHERE status in ('Active','".$lang['status_enum']['active']."') AND user.id<>'" . $_SESSION['UserId'] .
		...
		if ( $searchtxt == 'AA' ) {
			...
			$sql = 'SELECT DISTINCT user.*, floor(period_diff(extract(year_month from NOW()),extract(year_month from birth_date))/12) as age FROM ' . USER_TABLE .
			" as user WHERE status in ('Active','".$lang['status_enum']['active']."') AND user.id > 0 AND user.id<>'" . $_SESSION['UserId'] .
		...
		$querystring = array('searchtype'=>'simple','searchby'=>$searchby,'txtsearch'=>$searchtxt);
			...
			" as user, " . USER_PREFERENCE_TABLE . " as pref where user.id=pref.userid  AND user.id > 0 AND status in ('Active','".$lang['status_enum']['active']."') AND user.id<>'" . $_SESSION['UserId'] ."'";
			$keyval = '';
...
// . '%5B%5D=' .
	...
	if ( count($data) <= 0 ) {
		...
		$t->assign('rendered_page', $t->fetch('admin/showsimpsh.tpl') );
	...
	$sql = 'SELECT DISTINCT user.*, floor(period_diff(extract(year_month from NOW()),extract(year_month from birth_date))/12) as age FROM ! as user WHERE user.id > 0 AND ';
