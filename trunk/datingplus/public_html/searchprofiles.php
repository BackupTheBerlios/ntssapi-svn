...
if ( $searchtype == 'simple' ) {
	...
	if ( $searchby == 'username' ) {
			...
			$sql = 'SELECT DISTINCT user.*, floor(period_diff(extract(year_month from NOW()),extract(year_month from birth_date))/12) as age FROM ' . USER_TABLE .
			" as user, ".MEMBERSHIP_TABLE." as mem WHERE mem.roleid=user.level and mem.includeinsearch=1 and status in ('Active','".$lang['status_enum']['active']."') AND user.id > 0 AND user.id<>'" . $_SESSION['UserId'] .
		...
		$querystring = array('searchtype'=>'simple','searchby'=>$searchby,'txtsearch'=>$searchtxt);
			...
			$sql = 'SELECT  user.*,  floor(period_diff(extract(year_month from NOW()),extract(year_month from birth_date))/12) as age FROM ' . USER_TABLE .
			" as user, " . USER_PREFERENCE_TABLE . " as pref , ".MEMBERSHIP_TABLE." as mem WHERE mem.roleid=user.level and mem.includeinsearch=1 and user.id=pref.userid  AND status in ('Active','".$lang['status_enum']['active']."') AND user.id > 0 AND user.id<>'" . $_SESSION['UserId'] ."'";
			$keyval = '';
...
// . '%5B%5D=' .
	...
	$sql = 'SELECT DISTINCT user.*, floor(period_diff(extract(year_month from NOW()),extract(year_month from birth_date))/12) as age FROM ! as user, ! as mem WHERE mem.roleid=user.level and mem.includeinsearch=1 and user.id > 0 AND ';
