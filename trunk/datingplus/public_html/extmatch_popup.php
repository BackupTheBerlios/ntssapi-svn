...
if ( $_REQUEST['frm'] == 'frmExtSearch' && !isset($_REQUEST['sort']) ){
	...
	$sqlselect = 'SELECT DISTINCT user.*, floor(period_diff(extract(year_month from NOW()),extract(year_month from birth_date))/12) as age FROM ! user INNER JOIN ! pref ON user.id=pref.userid ';
