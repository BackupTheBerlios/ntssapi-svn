...
$sqlSelect = 'SELECT *, floor(period_diff(extract(year_month from NOW()),extract(year_month from birth_date))/12) as age'; 
...
$same_countrysql = ' FROM ! WHERE country = ? and username <> ? AND status in (\'Active\',\'' .$lang['status_enum']['active']. '\')' ;
$count_samecountrysql = $db->getOne( $CountSelect.$same_countrysql, array ( USER_TABLE, $user['country'], $user['username']  ) );

$same_statesql = ' FROM ! WHERE country = ? and state_province = ? and username <> ? AND status in (\'Active\',\'' .$lang['status_enum']['active']. '\')' ;
$count_samestatesql = $db->getOne( $CountSelect.$same_statesql , array( USER_TABLE, $user['country'], $user['state_province'], $user['username'] ) );

$same_countysql = ' FROM ! WHERE country = ? and state_province = ? and county = ? and username <> ? AND status in (\'Active\',\'' .$lang['status_enum']['active']. '\')' ;
...
$same_citysql = ' FROM ! WHERE country = ? and state_province = ? and county = ? and city = ? and username <> ? AND status in (\'Active\',\'' .$lang['status_enum']['active']. '\')' ;
...
$same_zipcodesql = ' FROM ! WHERE country = ? and state_province = ? and county = ? and city = ? and zip = ? and username <> ? AND status in (\'Active\',\'' .$lang['status_enum']['active']. '\')' ;
$count_samezipcodesql = $db->getOne( $CountSelect.$same_zipcodesql, array ( USER_TABLE, $user['country'], $user['state_province'], $user['county'], $user['city'], $user['zip'], $user['username']  ) );

$same_gendersql = ' FROM ! WHERE gender = ? and username <> ? AND status in (\'Active\',\'' .$lang['status_enum']['active']. '\')' ;
$count_samegendersql = $db->getOne( $CountSelect.$same_gendersql, array ( USER_TABLE, $user['gender'], $user['username']  ) );

$same_agesql = ' FROM ! WHERE username <> ? and floor(period_diff(extract(year_month from NOW()),extract(year_month from birth_date))/12) = ? AND status in (\'Active\',\'' .$lang['status_enum']['active']. '\')' ;
$count_sameagesql = $db->getOne( $CountSelect.$same_agesql, array ( USER_TABLE, $user['username'], $user['age'] ) );

//$same_firstnamesql = 'SELECT count( * ) FROM ! WHERE firstname = ?';

//$same_lastnamesql = 'SELECT count(*) FROM ! WHERE lastname = ?';

//$same_lookagestartsql = 'SELECT count(*) FROM ! WHERE lookagestart = ?';

//$same_lookageendsql = 'SELECT count(*) FROM ! WHERE lookageend = ?';

$same_lookagestartsql = ' FROM ! WHERE username <> ? and floor(period_diff(extract(year_month from NOW()),extract(year_month from birth_date))/12) >= ? AND status in (\'Active\',\'' .$lang['status_enum']['active']. '\')' ;
$count_samelookagestartsql = $db->getOne( $CountSelect.$same_lookagestartsql, array ( USER_TABLE, $user['username'], $user['lookagestart']  ) );

$same_lookageendsql = ' FROM ! WHERE username <> ? and floor(period_diff(extract(year_month from NOW()),extract(year_month from birth_date))/12) <= ? AND status in (\'Active\',\'' .$lang['status_enum']['active']. '\')' ;
...
$same_lookgendersql = ' FROM ! WHERE username <> ? AND status in (\'Active\',\'' .$lang['status_enum']['active']. '\')' ;
if ($user['lookgender'] == 'B') {
...
$same_lookcountrysql = ' FROM ! WHERE country = ? and username <> ? AND status in (\'Active\',\'' .$lang['status_enum']['active']. '\')' ;
$count_samelookcountrysql = $db->getOne( $CountSelect.$same_lookcountrysql, array ( USER_TABLE, $user['lookcountry'], $user['username']  ) );

$same_lookstatesql = ' FROM ! WHERE country = ? and state_province = ? and username <> ? AND status in (\'Active\',\'' .$lang['status_enum']['active'] . '\')' ;
...
$same_lookcountysql = ' FROM ! WHERE country = ? and state_province = ? and county = ? and username <> ? AND status in (\'Active\',\'' .$lang['status_enum']['active'] . '\')' ;
...
$same_lookcitysql = ' FROM ! WHERE country = ? and state_province = ? and county = ? and  city = ? and username <> ? AND status in (\'Active\',\'' .$lang['status_enum']['active']. '\')' ;
...
$same_lookzipsql = ' FROM ! WHERE country = ? and state_province = ? and county = ? and  city = ? and zip = ? and username <> ? AND status in (\'Active\',\'' .$lang['status_enum']['active']. '\')' ;
...
//$same_lookstatesql = 'SELECT count(*) FROM ! WHERE lookstate_province = ?';

//$same_timezonesql = 'SELECT count(*) FROM ! WHERE timezone = ?';
$same_looktimezonesql = ' FROM ! WHERE timezone = ? and username <> ? AND status in (\'Active\',\'' .$lang['status_enum']['active']. '\')' ;
$count_samelooktimezonesql = $db->getOne( $CountSelect.$same_looktimezonesql, array ( USER_TABLE, $user['timezone'], $user['username']  ) );

//$forum_postssql = 'SELECT user_posts FROM ! WHERE username = ?';

//$same_timeregistersql = 'SELECT count(*) FROM ! WHERE regdate = ?';

//$same_levelsql = 'SELECT count(*) FROM ! WHERE level = ?';
...
case 'samecity':
	...
	break;
case 'samezip':
	$data = $db->getAll ( $sqlSelect.$same_zipcodesql , array( USER_TABLE, $user['country'], $user['state_province'], $user['county'], $user['city'], $user['zip'], $user['username'] ) );
