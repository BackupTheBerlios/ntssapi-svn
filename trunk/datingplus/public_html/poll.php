...
if ( $_POST['action'] == $lang['savepoll'] ) {
	/* Add the question */
	
	if ($txtquestion == '') {
	
		$t->assign('error_msg', $lang['signup_js_errors']['question_noblank']);
	
	} elseif ( count($txtoptions) < 2) {
	
		$t->assign('error_msg', $lang['minimum_options'] );
		...
		$t->assign('error_msg', $lang['pollsuggested']);
