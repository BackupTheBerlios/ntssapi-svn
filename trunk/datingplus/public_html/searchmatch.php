...
if ( $with_photo == '1' ) {
	...
	$sql = 'SELECT distinct usr.*, floor(period_diff(extract(year_month from NOW()),extract(year_month from usr.birth_date))/12) as age FROM ! usr, ! as pics, ! as mem WHERE pics.userid = usr.id and mem.roleid=usr.level and mem.includeinsearch=1 AND usr.id > 0 and lower(usr.status) in (?, ?) AND floor(period_diff(extract(year_month from NOW()),extract(year_month from usr.birth_date))/12) BETWEEN ? AND ? ! ! ! ';

	$rs = $db->query( $sql, array(USER_TABLE, USER_SNAP_TABLE, MEMBERSHIP_TABLE, 'Active', $lang['status_enum']['active'], $_REQUEST['txtlookagestart'], $_REQUEST['txtlookageend'], $gender_search, $lookgender_search, $sortme ) );

} else {
	$sql = 'SELECT usr.*, floor(period_diff(extract(year_month from NOW()),extract(year_month from usr.birth_date))/12) as age FROM ! usr, ! as mem WHERE lower(usr.status) in(?, ?) AND mem.roleid = usr.level and mem.includeinsearch=1 AND usr.id > 0 AND floor(period_diff(extract(year_month from NOW()),extract(year_month from usr.birth_date))/12) BETWEEN ? AND ? ! ! ! ';

	$rs = $db->query( $sql, array(USER_TABLE, MEMBERSHIP_TABLE, 'Active', $lang['status_enum']['active'], $_REQUEST['txtlookagestart'], $_REQUEST['txtlookageend'], $gender_search, $lookgender_search, $sortme ) );
}
...
setcookie("osdate_info[search_ages]", $_REQUEST['txtlookagestart'].':'.$_REQUEST['txtlookageend'], strtotime("+30day"), "/" );

$querystring = array(
			'txtgender'			=> $_REQUEST['txtgender'],
			'txtlookgender'		=> $_REQUEST['txtlookgender'], 'txtlookagestart' 	=> $_REQUEST['txtlookagestart'], 'txtlookageend'		=> $_REQUEST['txtlookageend'],
			'with_photo'		=> $_REQUEST['with_photo']
			) ;
