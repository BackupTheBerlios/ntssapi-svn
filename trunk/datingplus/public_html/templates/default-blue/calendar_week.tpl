			...
			<table width="100%" border="0" cellpadding="0" cellspacing="0">
				<tr>
					...
					<td class="module_head" width="100%">
						...
						<img src="{$image_dir}view_w1.gif" alt="{$lang.view_week}" width="16" height="16" border="0" />&nbsp;&nbsp;&nbsp;
			...
			<table align="center" cellspacing="0" cellpadding="0" border="0">
				<tbody>
					<tr>
						...
						{if $prev != "" }
							<a href="?timestamp={$prev}&amp;calendarid={$calendarid}" > {$lang.previous_week}</a>&nbsp;
						{/if}
									</td>
						...
						{if $next != "" }
							 <a href="?timestamp={$next}&amp;calendarid={$calendarid}" >{$lang.next_week}</a>
						{/if}
									</td>
								</tr>
							</table>
						...
						<td>
							...
							{foreach item=item key=key from=$calendar}
								...
								{if $ccount>0 and $ccount<6}
									<td>{include file="calendar_weekday.tpl"}</td>
								{/if}
								{if $ccount==6}
									</tr>
								{/if}
						...
						<td>
							...
							{foreach item=item key=key from=$calendar}
								...
								{if $ccount==0 or $ccount==6}
									<td>{include file="calendar_weekend.tpl"}</td>
								{/if}
								{if $ccount==6}
									</tr>
								{/if}
