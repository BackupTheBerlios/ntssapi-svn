			...
			<table border="0" width="100%">
						...
						<table border="0">
							...
							<tr>
								...
								{if $config.enable_mod_rewrite == 'Y'}
									<a href="javascript:popUpScrollWindow2('{$item.id}.htm','top',650,600)">
								{else}
									<a href="javascript:popUpScrollWindow2('showprofile.php?id={$item.id}','top',650,600)">
								{/if}
				...
				<tr>
					<td colspan="2"  align="center" height="20" class="statusbar">
						{if $config.enable_mod_rewrite == 'Y'}
							<a href="javascript:popUpScrollWindow2('{$item.id}.htm','top',650,600)">{$lang.view_profile}</a>
						{else}
							<a href="javascript:popUpScrollWindow2('showprofile.php?id={$item.id}','top',650,600)">{$lang.view_profile}</a>
						{/if}
					</td>
				</tr>
			</table>
