...
{ldelim}
	var conmsg = "{$lang.admin_js__delete_error_msgs[13]}";
...
{ldelim}
	var conmsg = "{$lang.admin_js__delete_error_msgs[15]}";
	if (confirm(conmsg)){ldelim}
		...
		<td class="module_detail_inside" width="100%">
			<table width="100%" border="0" cellpadding="0" cellspacing="0">
				<tr>
					...
					{$lang.country_name}:&nbsp;{$countryname}&nbsp;&nbsp;>>&nbsp;&nbsp;
					{if $todo == 'add'}
						{$lang.insert_state}
					{elseif $todo == 'edit'}
						{$lang.modify_state}
					{else}
						{$lang.states_count}:&nbsp;{$total_recs}
					{/if}
					</td>
						...
						<table width="540" cellspacing="{$config.cellspacing}" cellpadding="{$config.cellpadding}" border="0">
							<tr>
								...
								<td align="left" width="150">
									{$lang.state_code}:<font color="{$lang.required_info_indicator_color}">{$lang.required_info_indicator}</font>
								</td>
								<td >
							...
							<tr>
								<td width="6">&nbsp;</td>
								<td width="150">
									{$lang.state_name}:<font color="{$lang.required_info_indicator_color}">{$lang.required_info_indicator}</font>
								</td>
								<td >
							...
							<tr>
								...
								<td align="center">
									...
									<input type="submit" class="formbutton" name="submit" {if $todo == 'edit'}value="{$lang.modify_state}"{else}value="{$lang.insert_state}" {/if} />
								</td>
							</tr>
						</table>
              </form>
		...
		{ else }
			<table width="540" border="0" cellspacing="0" cellpadding="0" align="center">
				<tr>
					<td>
						<table width="540" cellspacing="{$config.cellspacing}" cellpadding="{$config.cellpadding}" border="0">
							<tr><td colspan="3"><a href="managestates.php?countrycode={$countrycode}&amp;action=add">{$lang.insert_state}</a>
            ...
            <table width="540" cellspacing="{$config.cellspacing}" cellpadding="{$config.cellpadding}" border="0">
              <tr class="table_head">
                  ...
                  <th nowrap><a href="?sort={$lang.state_code|escape:url}&amp;type={$sort_type}&amp;countrycode={$countrycode}">{$lang.state_code}</a></th>
                  <th nowrap><a href="?sort={$lang.state_name|escape:url}&amp;type={$sort_type}&amp;countrycode={$countrycode}">{$lang.state_name}</a></th>
				...
				<tr>
					...
					{if $pageno != "1"}
						<a href="managestates.php?countrycode={$countrycode}&amp;offset={$pageno-1}">{$lang.previous}</a>&nbsp;
					{else}
						{$lang.previous}&nbsp;
					{/if}
					{if $total_recs > $page_size}
						<a href="managestates.php?countrycode={$countrycode}&amp;offset={$pageno+1}">{$lang.next}</a>&nbsp;
					{else}
						{$lang.next}&nbsp;
					{/if}
					</td>
