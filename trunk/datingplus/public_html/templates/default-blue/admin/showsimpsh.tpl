	...
	<tr>
		<td class="module_detail" width="571">
            ...
            <form action="" method="post">
							...
							<select name="sort_by">
								<option value="username" {if $sort_by eq 'username'}selected{/if}>{$lang.username|replace:':':''}</option>
								<option value="age" {if $sort_by eq 'age'}selected="selected"{/if}>{$lang.age}</option>
								<option value="logintime" {if $sort_by eq 'logintime'}selected="selected"{/if}>{$lang.logintime}</option>
							</select>&nbsp;
						...
						<td align="left">
							...
							{foreach item=item key=key from=$data}
								{if $ccount==0}
									<tr>
								{/if}
									<td align="left">{include file="admin/userresultviewsmall.tpl"}</td>
								{if $ccount==1}
									</tr>
								{/if}
			...
			<table align="center" border="0" cellpadding="0" cellspacing="0" >
				<tbody>
					<tr>
						{if $prev != "" }
						<td>
							<a href="?page={$prev}{foreach from=$querystring key=key item=val}&amp;{$key}={$val}{/foreach}">&lt;-- {$lang.previous}<a/>&nbsp;&nbsp;
						</td>
						{/if}
						{if $cpage != "" && $pages != "" }
						<td>
						...
						{if $next != "" }
						<td >
							&nbsp;&nbsp;<a href="?page={$next}{foreach from=$querystring key=key item=val}&amp;{$key}={$val}{/foreach}">{$lang.next} --&gt;</a>
						</td>
						{/if}
					</tr>
				</tbody>
