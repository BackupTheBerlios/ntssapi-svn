...
{ldelim}
	var conmsg = "{$lang.admin_js__delete_error_msgs[23]}";
...
{ldelim}
	var conmsg = "{$lang.admin_js__delete_error_msgs[24]}";
	if (confirm(conmsg)){ldelim}
...
<table width="571" border="0" cellpadding="0" cellspacing="0">
	<tr>
		...
		<td class="module_head" width="494">
			...
			{if $todo != ''} 
				<a href="managezips.php?countrycode={$countrycode}&amp;statecode={$statecode}&amp;countycode={$countycode}&amp;citycode={$citycode}" class="subhead">{$lang.manage_zips}</a>
			{else}
				{$lang.manage_zips}
			{/if}
		</td>
						...
						<table width="540" cellspacing="{$config.cellspacing}" cellpadding="{$config.cellpadding}" border="0">
							<tr>
								...
								<td align="left" width="150">
									{$lang.zip_code}:<font color="{$lang.required_info_indicator_color}">{$lang.required_info_indicator}</font>
								</td>
								<td >
							...
							<tr>
								...
								<td align="center">
									...
									<input type="submit" class="formbutton" name="submit" {if $todo == 'edit'}value="{$lang.modify_zip}"{else}value="{$lang.insert_zip}" {/if} />
								</td>
							</tr>
						</table>
              </form>
		...
		{ else }
			<table width="540" border="0" cellspacing="0" cellpadding="0" align="center">
				<tr>
					<td>
						<table width="540" cellspacing="{$config.cellspacing}" cellpadding="{$config.cellpadding}" border="0">
							<tr><td colspan="3"><a href="managezips.php?countrycode={$countrycode}&amp;statecode={$statecode}&amp;countycode={$countycode}&amp;citycode={$citycode}&amp;action=add">{$lang.insert_zip}</a>
            ...
            <table width="540" cellspacing="{$config.cellspacing}" cellpadding="{$config.cellpadding}" border="0">
							<tr class="table_head">
			  					...
			  					<th nowrap><a href="?sort={$lang.city_code|escape:url}&amp;type={$sort_type}&amp;countrycode={$countrycode}&amp;statecode={$statecode}&amp;countycode={$countycode}&amp;citycode={$citycode}">{$lang.zip_code}</a></th>
				...
				<tr>
					...
					{if $pageno != "1"}
						<a href="managezips.php?countrycode={$countrycode}&amp;statecode={$statecode}&amp;countycode={$countycode}&amp;citycode={$citycode}&amp;offset={$pageno-1}">{$lang.previous}</a>&nbsp;
					{else}
						{$lang.previous}&nbsp;
					{/if}
					{if $total_recs > $page_size}
						<a href="managezips.php?countrycode={$countrycode}&amp;statecode={$statecode}&amp;countycode={$countycode}&amp;citycode={$citycode}&amp;offset={$pageno+1}">{$lang.next}</a>&nbsp;
					{else}
						{$lang.next}&nbsp;
					{/if}
					</td>
						...
						<table width="540" border="0" cellspacing="2" cellpadding="1" align="center">
							<tr>
								...
								<form name="srchMe" method="post" action="">
									...
									<input type="hidden" name="citycode" value="{$citycode}" />
									{$lang.zip_code}:&nbsp;
									<input name="zipcode" value="{$zipcode}" type="text" size="10" maxlength="20" />
									&nbsp;
