...
<table width="571" border="0" cellpadding="0" cellspacing="0">
	<tr>
		...
		<td class="module_head" width="494">
			...
			{if $todo != ''}
				<a href="managecounties.php?countrycode={$countrycode|escape:url}&amp;statecode={$statecode}" class="subhead">{$lang.manage_counties}</a>
				{* &nbsp;>&nbsp;
				{if $todo == 'add'}
					{$lang.insert_county}
				{else}
			...
			<table width="100%" border="0" cellpadding="0" cellspacing="0">
				<tr>
					...
					{elseif $todo == 'edit'}
						{$lang.modify_county}
					{else}
						{$lang.counties_count}:&nbsp;{$total_recs}
					{/if}
					</td>
						...
						<table width="540" cellspacing="{$config.cellspacing}" cellpadding="{$config.cellpadding}" border="0">
							<tr>
								...
								<td align="left" width="150">
									{$lang.county_code}:<font color="{$lang.required_info_indicator_color}">{$lang.required_info_indicator}</font>
								</td>
								<td >
							...
							<tr>
								<td width="6">&nbsp;</td>
								<td width="150">
									{$lang.county_name}:<font color="{$lang.required_info_indicator_color}">{$lang.required_info_indicator}</font>
								</td>
								<td >
							...
							<tr>
								...
								<td align="center">
									...
									<input type="submit" class="formbutton" name="submit" {if $todo == 'edit'}value="{$lang.modify_county}"{else}value="{$lang.insert_county}" {/if} />
								</td>
							</tr>
						</table>
              </form>
		...
		{ else }
			<table width="540" border="0" cellspacing="0" cellpadding="0" align="center">
				<tr>
					<td>
						<table width="540" cellspacing="{$config.cellspacing}" cellpadding="{$config.cellpadding}" border="0">
							<tr><td colspan="3"><a href="managecounties.php?countrycode={$countrycode|escape:url}&amp;statecode={$statecode}&amp;action=add">{$lang.insert_county}</a>
            ...
            <table width="540" cellspacing="{$config.cellspacing}" cellpadding="{$config.cellpadding}" border="0">
							<tr class="table_head">
			  					...
			  					<th nowrap><a href="?sort={$lang.county_code|escape:url}&amp;type={$sort_type}&amp;countrycode={$countrycode|escape:url}&amp;statecode={$statecode|escape:url}">{$lang.county_code}</a></th>
			  					<th nowrap><a href="?sort={$lang.county_name|escape:url}&amp;type={$sort_type}&amp;countrycode={$countrycode}&amp;statecode={$statecode}">{$lang.county_name}</a></th>
				...
				<tr>
					...
					{if $pageno != "1"}
						<a href="managecounties.php?countrycode={$countrycode}&amp;statecode={$statecode}&amp;offset={$pageno-1}">{$lang.previous}</a>&nbsp;
					{else}
						{$lang.previous}&nbsp;
					{/if}
					{if $total_recs > $page_size}
						<a href="managecounties.php?countrycode={$countrycode}&amp;statecode={$statecode}&amp;offset={$pageno+1}">{$lang.next}</a>&nbsp;
					{else}
						{$lang.next}&nbsp;
					{/if}
					</td>
