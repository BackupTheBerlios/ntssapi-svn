	...
	<tr>
		<td class="module_detail_inside" width="100%">
			...
			<table width="100%" border="0" cellpadding="{$config.cellpadding}"  cellspacing="{$config.cellspacing}">
				<tr><td width="35%">{$lang.cc_owner}</td><td width="65%"><input type="text" name="paypal_ccowner"/></td></tr>
				<tr><td>{$lang.cc_number}</td><td><input type="text" name="paypal_ccnumber"/></td></tr>
				<tr><td>{$lang.cc_exp_date}</td>
					<td>
						{html_select_date_translated display_days=false end_year=+10 prefix="paypal_exp_"}
					</td>
				</tr>
				<tr><td>{$lang.cc_cvv_number}</td><td><input type="text" name="paypal_cvvnumber" size="4" maxlength="3"/>&nbsp;{$lang.cvv_help}</td></tr>
			</table>
			{/if}
		</td>
	</tr>
