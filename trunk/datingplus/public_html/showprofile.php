...
if( $_GET['username'] != '') {
	$userid = $db->getOne( 'SELECT id FROM ! WHERE username = ? AND status = ? AND allow_viewonline = ?', array( USER_TABLE, $_GET['username'], 'active', '1' ) );
	$_GET['id'] = $userid;
}
...
*/
	...
	$t->assign('profile_views', $db->getOne($sql, array( VIEWS_WINKS_TABLE, $_GET['id'], 'V' ) ) );
