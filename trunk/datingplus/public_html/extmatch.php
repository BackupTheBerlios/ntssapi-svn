...
if ( $_REQUEST['frm'] == 'frmExtSearch' && !isset($_REQUEST['sort']) ){
	...
	$sqlselect = 'SELECT DISTINCT user.*, floor(period_diff(extract(year_month from NOW()),extract(year_month from birth_date))/12) as age FROM ! user INNER JOIN ! pref ON user.id=pref.userid JOIN '.MEMBERSHIP_TABLE.' as mem ON  user.level=mem.roleid and mem.includeinsearch = 1 ';

//	$sqlwhere = ' WHERE user.lookgender in '. $txtgender_search;

	$sqlwhere = " WHERE user.id > 0 AND status in ('Active','".$lang['status_enum']['active']."') ";
